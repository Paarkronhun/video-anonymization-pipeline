...
def run_pipeline(input_source: str, output_path: str, anon_mode: str):
    ...
    try:
        while True:
            ...
            anonymized_frame = anonymize_frame(frame=frame, mode=anon_mode)
            out.write(anonymized_frame)

            frame_count += 1
            if frame_count % 30 == 0:
                pct = round(frame_count / total_frames * 1000) / 10
                logging.info(f"Processed {frame_count}/{total_frames} frames | Progress {pct} %")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="YOLO Video Anonymization Pipeline")
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["face", "body"],
        help="face = censor only the face, body = censor the whole person",
    )
